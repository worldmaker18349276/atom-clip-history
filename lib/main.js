...
module.exports = {
  config: {
    ...
    selectPasted: {
      ...
      type: "boolean",
      default: false,
      description: "Select last pasted text"
    },
  ...
  activate() {
    ...
    this.disposables.add(atom.commands.add("atom-text-editor", {
      "clip-history:paste": makeCommandCallback(editor => this.paste(editor, PASTE.OLDER)),
      "clip-history:paste-newer": makeCommandCallback(editor => this.paste(editor, PASTE.NEWER)),
      "clip-history:paste-last": makeCommandCallback(editor => this.paste(editor, PASTE.LAST)),
      "clip-history:clear": makeCommandCallback(editor => this.clear()),
    }))
  },
  ...
  paste(editor, which) {
    ...
    const select = atom.config.get("clip-history.selectPasted")
    const autoIndent = atom.config.get("editor.autoIndentOnPaste")
    const flash = which !== PASTE.LAST && atom.config.get("clip-history.flashOnPaste")
