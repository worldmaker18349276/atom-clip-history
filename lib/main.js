...
class History {
  ...
  insertText(cursor, text) {
    ...
    if (this.pasteArea.has(cursor))
        cursor.selection.setBufferRange(this.pasteArea.getRange(cursor))
    const marker = editor.markBufferRange(cursor.selection.insertText(text, {autoIndent:true}))
